...
jQuery.toCSS = function( style ) {
  ...
  **/
  ...
  var toString = function( allStyles ) {
    var allString = '';
    // convert allStyles to a string
    for ( key in allStyles ) {
      var selectorString = $.trim( key ) + ' {\n';
      var attributesString = '';
      for ( attribute in allStyles[key] ) {
        attributesString += '  ' + $.trim( attribute ) + ': ' + $.trim( allStyles[key][attribute] ) + ';\n'
      }
      if ( attributesString != '' )
        allString += selectorString + attributesString + '}\n\n';
    }
    return allString;
  }

  applyStyles( '', style );
  return toString( allStyles );
}
